<div class="section tab-pane" id="invite-members">
    <h3 class="box-title text-center invite-heading margin-bottom-md">Invite Members</h3>
    <!-- /.box-header -->
    <div class="box-body">
        <div class="col-md-8 col-md-offset-2">
            <form method="POST" action="{{ url('teams/'. $team->id .'/invite-members') }}" id="add_users_one_at_a_time">
                    ...
                    <div class="row item-to-clone padding-top-sm padding-bottom-sm border-radius-xs">
                        <div class="col-sm-6 col-xs-11 form-group no-bottom-margin has-feedback {{ $errors->has('email') ? 'has-error' : '' }}">
                            <div class="input-group">
                                <input type="email" name="email[]" class="form-control text_input clear-after" value="{{ old('email[0]') }}" placeholder="Email">
                                <span class="input-group-addon input-addon-email padding-left-right-phone" title="Invite Using Phone Number"><i class="fa fa-mobile"></i></span>
                            </div>
                            @if ($errors->has('email'))
                        ...
                        <div class="col-sm-6 col-xs-11 form-group no-bottom-margin has-feedback hidden {{ $errors->has('phone') ? 'has-error' : '' }}">
                            <div class="input-group">
                                <input type="phone" name="phone[]" class="form-control text_input clear-after" value="{{ old('phone[0]') }}" placeholder="Phone">
                                <span class="input-group-addon input-addon-phone" title="Invite Using Email Address"><i class="fa fa-envelope" ></i></span>
                            </div>
                            @if ($errors->has('phone'))
                                <span class="help-block">
                                    {{ $errors->first('phone') }}
                ...
                <div class="add-more-contain">
                    ...
                    <span>or</span> <a href="#" class="no-underline add_several_users" title="Add Several At Once">Add several</a>
                </div>
                <textarea class="form-control countingdown" placeholder="Add Custom Invite Message (140 Characters Max)" maxlength="140" rows="2"></textarea>
...
@section('added_js')
    <script>
        ...
        $('.input-addon-email').click(function() {
            ...
            mainParent.next('div.form-group').find('.form-control').val('');
            mainParent.next('div.form-group').removeClass('hidden');
        });
        $('.input-addon-phone').click(function() {
            var mainParent = $(this).parents('div.form-group');
            mainParent.addClass('hidden');
            mainParent.prev('div.form-group').find('.form-control').val('');
